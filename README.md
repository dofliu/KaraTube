...
│       ├── settings.py          # 系統設定（預設調音、快取上限、模型選擇）
...
# 前端純邏輯（26 條，用 Node 內建測試執行器，不需要 npm install）
...
| 系統設定面板（⚙️） | 開機預設調音、自動音量平衡（開關/目標響度）、快取上限與自動清理、Whisper / Demucs 模型、片頭卡與結算畫面秒數 |
